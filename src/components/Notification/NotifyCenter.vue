      ...
      <div class="tool-bar">
        <el-row>
          <el-col :span="16">
            <el-radio-group v-model="searchForm.module" size="mini" @input="handleInput">
              <el-radio-button :label="item.code" :name="item.code" v-for="item in notificationModuleG"
                               :key="item.code">
          ...
          <el-col :span="8">
            <el-select v-model="searchForm.isRead" :style="{width: '100px'}" placeholder="请选择" style="float: right"
                       @change="handleInput">
              <el-option
        ...
        <el-table
            ...
            <el-table-column
                ...
                label="是否已读">
                <template slot-scope="scope">
                    <el-button v-if="scope.row.isRead === 1" @click="rowClick(scope.row)" size="mini" type="text"
                               :loading="scope.row.readLoading">
                          <span class="read_flag">已读</span>
                    </el-button>
                    <el-button @click="rowClick(scope.row)" v-else size="mini" type="text"
                               :loading="scope.row.readLoading">
